...
// Start a session
session_start();
echo '   alert("Inside Mailto PHP code");';
...
$featuredItems = array(
    "Electronics" => array(
        array(
            "name" => "Phone",
            "link" => "products.php",
            "image" => "img/emailpics/smartphone.jpg"
        ),
        array(
            "name" => "Laptop",
            "link" => "products.php",
            "image" => "img/emailpics/gaming-laptop.jpg"
        ),
        array(
            "name" => "Router",
            "link" => "products.php",
            "image" => "img/emailpics/router.jpg"
        ),
        array(
            "name" => "Switch",
            "link" => "products.php",
            "image" => "img/emailpics/unifi_switch.jpg"
        )
    ),
...
function sendRegistrationEmail($to, $userName, $featuredItems) {
    ...
    // Initialize an empty array to store embedded images
    $embeddedImages = array();

    // Process each item to embed images
    foreach ($featuredItems as $category => $items) {
        if (is_array($items)) {
            foreach ($items as $key => $item) {
                if (is_array($item) && isset($item['image']) && file_exists($item['image'])) {
                    // Read image content and encode it as base64
                    $imageData = file_get_contents($item['image']);
                    $base64Image = base64_encode($imageData);

                    // Determine MIME type based on image file extension
                    $imageFileType = pathinfo($item['image'], PATHINFO_EXTENSION);
                    $mimeType = 'image/' . strtolower($imageFileType);

                    // Create data URI for the image
                    $embeddedImages[$item['image']] = "data:$mimeType;base64,$base64Image";

                    // Replace image source with data URI in $featuredItems array
                    $featuredItems[$category][$key]['image'] = $embeddedImages[$item['image']];
                }
            }
        }
    }
    ...
    // Replace image source URLs in HTML content with base64 encoded images
    foreach ($embeddedImages as $imagePath => $dataUri) {
        $message = str_replace($imagePath, $dataUri, $message);
    }
